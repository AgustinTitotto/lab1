...
<script>
    ...
    let myCounter = 0
    let desc = [<#list descriptions as description>"${description.gamerUser.userName}, ${description.game.gameName}, ${description.lvl}, ${description.rank.rankName}", </#list>]
    class Carousel {
            ...
            if (e.isFinal) {
                ...
                if (successful) {
                    ...
                    if (this.startPosX < posX) {
                        ...
                        xhr.open('POST', 'http://localhost:4335/tinderswipe', true)
                        xhr.setRequestHeader("Content-Type", "application/xhtml+xml; charset=utf-8")

                        const data = desc[myCounter]
                        xhr.send(data)
                    }
        ...
        push() {
            ...
            if (desc.length > myCounter) {
                let card = document.createElement('div')

                card.classList.add('card')

                card.innerText = desc[myCounter]
